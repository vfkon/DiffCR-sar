...
class CBAM_mod(nn.Module):
    def __init__(self, channels, out_channels, reduction=4, bias = False, window = 7):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels=12, out_channels=out_channels,
...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        #                       bias=True)
        self.cond_intro = CBAM_mod(img_channel + 3, width)
        ...
        self.residual_ending = CBAM_mod(img_channel + 3, width)
    ...
    def forward(self, inp, gammas):
        ...
        res_end = self.residual_ending(inp[:, :6])
